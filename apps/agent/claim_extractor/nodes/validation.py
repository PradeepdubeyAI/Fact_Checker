...
async def _validate_claim(potential_claim: PotentialClaim) -> ValidatedClaim:
    ...
    llm = get_llm(stage="claim_extractor.validation")  # Uses default temperature for consistent results
...
async def validation_node(state: State) -> Dict[str, Sequence[ValidatedClaim]]:
    ...
    if not VALIDATION_CONFIG.get("enabled", True):
        logger.info("Validation disabled - auto-accepting all claims as valid")
        # Auto-validate all claims without LLM calls
        validated_claims = []
        seen_claims = set()
        
        for claim in potential_claims:
            if claim.claim_text and claim.claim_text not in seen_claims:
                validated_claims.append(
                    ValidatedClaim(
                        claim_text=claim.claim_text,
                        is_complete_declarative=True,  # Assume valid since Decomposition creates complete sentences
        ...
        logger.info(f"Auto-validated {len(validated_claims)} unique claims (validation disabled)")
