...
async def sentence_splitter_node(state: State) -> Dict[str, List[ContextualSentence]]:
    ...
    """
    # Use preprocessed text if available, otherwise fall back to original
    text_to_process = state.preprocessed_text or state.answer_text
    ...
    contextual_sentences = await _sentence_splitter_and_context_creator(
        text_to_process, p_sentences, f_sentences, bool(metadata), metadata
    )
