...
class SearchProviders:
    ...
    async def exa(query: str) -> List[Evidence]:
        if not HAS_EXA:
            logger.warning("Exa is not installed. Install with: pip install langchain-exa")
            return []
    ...
    async def tavily(query: str) -> List[Evidence]:
        ...
        try:
            # Record Tavily API call
            tracker = get_metrics_tracker()
            tracker.record_tavily_call()
            logger.info(f"Tavily API call recorded. Current total: {tracker.tavily_calls}")
